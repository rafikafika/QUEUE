...
void transaksiObat(){
	...
	if(flagObat == 0) {
		...
		cout << "Masukan Banyak Transaksi Obat > ";
		cin >> n;
	if (n == 0) {
		cout << "Input salah!";
	} else {
		for (int i = 0 ; i < n ; i = i + 1){
			cout << "Masukan Id Obat > ";
			cin >> id;
			cout << "Masukan Banyak Obat > ";
			cin >> x;
...
void cariDataObat(){
	...
	if(flagObat == 0) {
		...
		tampilObat();
		cout << endl << "Masukan id obat yang ingin di cari > ";
		cin >> id;
		cout << endl << "=========================" << endl;
